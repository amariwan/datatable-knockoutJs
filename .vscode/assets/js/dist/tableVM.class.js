class tableVM {
	constructor(url, classID) {
		...
		//Test
		self.deleteRecordsRows = function () {
			console.log('deleteRecordsRows', self.table.selectedRows());

			// self.server.getDeletedRecords(self.loadDeletedRecords);
		};

		self.insertRecordsRows = function () {
			self.insertRecords = ko.observableArray([]);
			ko.utils.arrayFirst(self.rows(), function (data, i) {
				if (data.isSelectedRows() === true && self.insertRecords.indexOf(data) < 0) {
					self.insertRecords.push(data);
				}
			});
			// self.server.getInsertedRecords(self.loadInsertedRecords);
		};

		self.updateRecordsRows = function () {
			self.updateRecords = ko.observableArray([]);
			ko.utils.arrayFirst(self.rows(), function (data, i) {
				if (data.isSelectedRows() === true && self.updateRecords.indexOf(data) < 0) {
					self.updateRecords.push(data);
				}
			});
			// self.server.getUpdatedRecords(self.loadUpdatedRecords);
		};
		...
		self.createTable = async (nodes, propertys) => {
			...
			self.table = new createTable({
				...
				url: url,
				isSelectedAllRowsChecked: self.isSelectedAllRowsChecked,
				selectedRowsValue: self.selectedRowsValue,
				rows: self.rows,
			});
		...
		self.selectedAllRows = () => {
			if (self.isSelectedAllRowsChecked()) {
				self.table.selectedAllRows();
		...
		self.cleanSelectedRows = () => {
			self.selectedRowsValue.removeAll();
			log('clean');
			// self.updateRows(self.backUpRows());
		};
		self.selectedRowsValue = ko.observableArray();
		...
		self.checkList3 = () => {
			$('.checkList3' + self.classID()).toggleClass('visible');
		};
