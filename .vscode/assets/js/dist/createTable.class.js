...
class createTable {
	constructor(params) {
		...
		self.isSelectedAllRowsChecked = params.isSelectedAllRowsChecked;
				...
				data.forEach((datum, i) => {
					...
					columns.forEach((column) => {
						...
						if (typeof datum[column] === 'object') {
							...
							var id = column + i;
							row.isSelectedRows = id;
							row.refresh = () => {
								log(params.selectedRowsValue());
								log(self.selectedRowsValue());
								if (row.isSelectedRows) {
									self.selectedRows.push(datum);
								} else {
									self.selectedRows.remove(datum);
								}
								if (params.selectedRowsValue().length === 0) {
									$('.isCanShowSelectedRows' + self.classID()).addClass('hidden');
								} else if (params.selectedRowsValue().length === data.length) {
									console.log('All selected');
									self.isSelectedAllRowsChecked(true);
								} else {
									$('.isCanShowSelectedRows' + self.classID()).removeClass('hidden');
									self.isSelectedAllRowsChecked(false);
								}
								console.log('Selected', self.selectedRows());
							};
						} else {
		...
		// 	read: function () {
		// 		log('bbb');
		// 		return self.selectedRows().length === self.rows().length;
		// 	},
		// 	write: function (newVal) {
		// 		log(newVal);
		// 		if (newVal === self.selectAll()) return;
		// 		if (!!newVal) self.selectedRows(getAllCodes());
		// 		if (!newVal) self.selectedRows([]);
		// 	},
		...
		self.selectedAllRows = async () => {
			console.log('Selected all rows');
			var data = self.updateParamData();
			data = self.rows().map((f, i) => {
				if (params.selectedRowsValue()[i] != f.isSelectedRows) params.selectedRowsValue.push(f.isSelectedRows);
				return;
			});
			log(params.selectedRowsValue());
		};
		...
		self.getKey = async (key, doReset) => {
			log(key, doReset);
			var body = { objtype: 'srvReqKey', key: key, session: sessionStorage.getItem('glb_sessionUID') };
			var data = await connectToServer(self.url.sub, 'post', true, body);
			log(data.type[0]);
